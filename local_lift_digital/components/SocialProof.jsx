      ...
      <div className="mx-auto max-w-7xl px-6 py-12 sm:py-14 lg:px-8">
        ...
        <div className="mt-9 grid grid-cols-2 items-center gap-6 sm:grid-cols-3 lg:grid-cols-5 lg:gap-8">
          {businesses.map((business) => (
            <div
              key={business}
              className="flex min-h-16 items-center justify-center rounded-lg border border-slate-800 px-4 py-4 text-center transition-colors duration-200 hover:border-slate-700"
            >
              <span className="text-sm font-bold tracking-tight text-slate-300 sm:text-base">
                {business}
              </span>
            </div>
          ))}
